...
########
# Inputs:
R = 330           # Probe (Tungsten) Resistance (ohm)
...
for f in files:
    ...
    b = abs(popt[1])
    mi = 6.6335209E-26      # Mass of H3+ [kg]
    #mi = 3*1.6735575E-27
    ni = (b * np.sqrt(mi)) / (2 * e * (D/2) * L * np.sqrt(2*e))
    Ehump = E_range[np.argmax(EEPF)]
    print('Electron Density (1/m3)' , ne*1.6720640371455053)
    print('Ion Density (1/m3)' , ni)
    print('Electron Temperature (eV)', Te)
    print('Hump Energy', Ehump)
    ...
    EEP_F.append(EEPF)
    E_f.append(E_range)
print(np.multiply(n_e,1.6720640371455053))
print(n_i)
print(T_e)


#np.savetxt('ne_ni_Te_hu.txt', np.column_stack([n_e,n_i,T_e,E_hump]), delimiter = ',')
